...
# figsize を縮小してスケール係数を改善 (scale = 4.82" / 10" = 0.48)
fig, ax = plt.subplots(figsize=(10, 4.375))
ax.set_xlim(0, 16)
ax.set_ylim(0, 7)
ax.axis("off")
...
def label(ax, x, y, text, size=13, color="black", bold=False, va="center", ha="center", zorder=4):
...
label(ax, 4.3, 5.65, "Dual-Stream Embedding", size=11, bold=True, color="#1d4ed8")

# Encoder section
box(ax, 8.9, 1.2, 3.2, 4.6, C_BG_ENC, "#6ee7b7", lw=1.0, radius=0.3, alpha=0.5)
label(ax, 10.5, 5.65, "Transformer Encoder", size=11, bold=True, color="#065f46")

# Output section
box(ax, 12.5, 1.2, 3.1, 4.6, C_BG_HEAD, "#d1d5db", lw=1.0, radius=0.3, alpha=0.5)
label(ax, 14.05, 5.62, "Output Heads", size=11, bold=True, color="#374151")
...
label(ax, 0.90, 3.7, "Input", size=13, bold=True, color=C_INPUT)
label(ax, 0.90, 3.3, r"$x_1, \ldots, x_L$", size=13, color=C_INPUT)
...
label(ax, 2.90, 4.88, "Mag. Features", size=12, bold=False, color=C_MAG)
label(ax, 2.90, 4.55, r"$f_\mathrm{mag} \in \mathbb{R}^4$", size=12, color=C_MAG)

# Modulo feature box
box(ax, 1.95, 2.85, 1.9, 0.8, "#ffedd5", C_MOD, lw=1.5, radius=0.12)
label(ax, 2.90, 3.38, "Mod. Features", size=12, bold=False, color=C_MOD)
label(ax, 2.90, 3.05, r"$f_\mathrm{mod} \in \mathbb{R}^{200}$", size=12, color=C_MOD)
...
label(ax, 4.90, 4.80, r"MLP$_\mathrm{mag}$", size=13, bold=True, color=C_MAG)
label(ax, 4.90, 4.52, r"$h_\mathrm{mag} \in \mathbb{R}^d$", size=12, color=C_MAG)

# W_mod
box(ax, 4.15, 2.85, 1.5, 0.8, "#fed7aa", C_MOD, lw=1.5, radius=0.12)
label(ax, 4.90, 3.30, r"$W_\mathrm{mod}$", size=13, bold=True, color=C_MOD)
label(ax, 4.90, 3.02, r"$h_\mathrm{mod} \in \mathbb{R}^d$", size=12, color=C_MOD)
...
label(ax, 7.32, 5.22, "FiLM Fusion", size=12, bold=True, color=C_FILM)
label(ax, 7.32, 4.80, r"$e_i = (1+\gamma)\odot h_\mathrm{mag} + \beta$",
      size=11, bold=True, color=C_FILM)
label(ax, 7.32, 4.42, r"$e_i \in \mathbb{R}^d$", size=12, color=C_FILM)
label(ax, 7.32, 4.07, r"$+ \;\mathrm{PE}$", size=12, color="#7c3aed")
...
# small label on arrows (x positions pulled left of FiLM box border at x=6.05)
ax.text(5.72, 4.84, r"$h_\mathrm{mag}$", fontsize=10, color=C_MAG,
        ha="center", va="bottom", zorder=5)
ax.text(5.65, 3.55, r"$h_\mathrm{mod}$", fontsize=10, color=C_MOD,
        ha="center", va="bottom", zorder=5)
...
label(ax, 10.35, 4.35, r"Pre-LN", size=13, bold=True, color=C_ENC)
label(ax, 10.35, 4.02, r"Multi-Head Attn", size=12, color=C_ENC)
label(ax, 10.35, 3.72, r"FFN", size=13, color=C_ENC)
label(ax, 10.35, 3.40, r"$\times N$ layers", size=13, bold=True, color="#065f46")
...
for (y0, fc, ec, title, subtitle) in HEAD_SPECS:
    ...
    label(ax, HEAD_X + HEAD_W/2, y0 + 0.78, title, size=12, bold=True, color=ec)
    label(ax, HEAD_X + HEAD_W/2, y0 + 0.32, subtitle, size=11, color="#374151")
...
ax.legend(handles=legend_items, loc="lower left", bbox_to_anchor=(0.01, 0.01),
          fontsize=11, framealpha=0.9, ncol=5,

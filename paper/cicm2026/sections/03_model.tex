...
three sign classes; $\mathcal{L}_{\text{mod}}$ is the mean cross-entropy of the 100 Modulo
heads, normalised by $\log m$: $\mathcal{L}_{\text{mod}} = \frac{1}{100}\sum_{m=2}^{101}
\frac{1}{\log m}\mathcal{L}_{\text{CE}}^{(m)}$.
...
Each candidate $n$ is scored by
$\text{score}(n) = -(v_n - \mu_i)^2/(2\sigma_i^2) + 0.3\sum_{m}\log P(n \bmod m)$;

...
<!-- 図のプレースホルダー -->
<!-- 図5（散布図）: 予測 Magnitude vs. 真の Magnitude（対数スケール）。Large モデル 3 バリアント。
     各バケット（Small/Medium/Large/Huge/Astronomical）を色分けする。 -->
...
<!-- 図のプレースホルダー -->
<!-- 図6（校正図）: 予測信頼度（confidence）vs. 実際の正解率（accuracy）の校正曲線。
     出典: checkpoints/large_std/{model}/analysis/magnitude/calibration_plot.png -->

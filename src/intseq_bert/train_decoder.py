...
Compatible with Dynamic Confidence-Ordered CRT.
"""
...
def get_targets(integers: List[int]) -> Dict[str, torch.Tensor]:
    ...
    # Modulo residues (Python % handles negatives correctly for math mods)
    mod3 = torch.tensor([x % 3 for x in integers], dtype=torch.long)
    mod5 = torch.tensor([x % 5 for x in integers], dtype=torch.long)
    mod7 = torch.tensor([x % 7 for x in integers], dtype=torch.long)
    mod8 = torch.tensor([x % 8 for x in integers], dtype=torch.long)
    mod10 = torch.tensor([x % 10 for x in integers], dtype=torch.long)
    mod11 = torch.tensor([x % 11 for x in integers], dtype=torch.long)
    mod13 = torch.tensor([x % 13 for x in integers], dtype=torch.long)
    mod100 = torch.tensor([x % 100 for x in integers], dtype=torch.long)
...
def evaluate_decoder(
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            # Calculate predicted magnitude (log scale) from bins
            ...
            for head in correct_counts.keys():
            ...
            # batch_reconstruct now uses the Dynamic Confidence-Ordered CRT logic
            ...
            for i in range(batch_size):
                ...
                pred_mag_val = mag_pred[i].item()
                mag_error = abs(pred_mag_val - true_mag)
                
                if recon_int == true_int:
                    if mag_error > 0.5:
                        # Rescued: Bin prediction was off by >20 bins, but CRT fixed it!
                        rescued_count += 1
                    else:
                        # Perfect: Everything aligned
                        perfect_count += 1
                else:
                    failed_count += 1
    ...
    logger.info(f"  Mod Accuracies: " + 
                f"3:{accuracies['mod3_acc']:.1f}% | " +
                f"7:{accuracies['mod7_acc']:.1f}% | " +
                f"8:{accuracies['mod8_acc']:.1f}% | " +
                f"11:{accuracies['mod11_acc']:.1f}% | " +
                f"13:{accuracies['mod13_acc']:.1f}% | " +
                f"100:{accuracies['mod100_acc']:.1f}%")
    
    logger.info(f"  Reconstruction (n={total_samples}):")
    logger.info(f"    ✓ Perfect: {perfect_count} ({perfect_count/total_samples*100:.1f}%)")
    logger.info(f"    ✓ Rescued: {rescued_count} ({rescued_count/total_samples*100:.1f}%)  ← CRT Logic Active")
...
def train_decoder(config: Dict[str, Any]) -> None:
    ...
    logger.info("=" * 50)
    logger.info("Starting Decoder Training (v2: Dynamic CRT)")
    logger.info("=" * 50)
    ...
    if bypass_bert:
        logger.info("BYPASS MODE ENABLED: Learning from raw features (Sanity Check)")
    else:
    ...
    # Initialize decoder (35-dim input)
    decoder = NumberTheoreticDecoder(input_dim=35).to(device)
    logger.info(f"Decoder initialized with 35-dim input")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            loss = (
                ...
                5.0 * F.cross_entropy(decoder_output['mag'], targets['mag'].to(device)) +
    ...
    evaluate_decoder(bert_model, decoder, test_loader, device, logger)
